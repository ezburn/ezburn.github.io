...
//     Load the "ezburn-wasm" package from "http://localhost:8080" (for local development)
//
...
async function reloadWorker(version: Version): Promise<Worker> {
  ...
  try {
    ...
    if (version === 'pkgurl') {
      ...
      promiseWASM = fetch(new URL(`ezburn.wasm`, pkgurlParam!))
    }
      ...
      promiseJS = packageFetch(`ezburn-wasm@${version}/lib/browser${min}.js`)
      promiseWASM = packageFetch(`ezburn-wasm@${version}/ezburn.wasm`)
    }
    ...
    return await new Promise<Worker>((resolve, reject) => {
      ...
      worker.postMessage([version, ezburnWASM], [ezburnWASM])
    })
  }
