// This file contains a hack to get the "ezburn-wasm" package to run in the
...
// The "ezburn-wasm" package overwrites "fs.writeSync" with this value
let ezburnWriteSync: (
  fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null
) => void

// The "ezburn-wasm" package overwrites "fs.read" with this value
let ezburnRead: (
  fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null,
...
function writeSync(
  ...
  if (fd <= 2) {
    ...
    else ezburnWriteSync(fd, buffer, offset, length, position)
  } else {
    throw EINVAL
  }
}
...
function read(
  ...
  if (fd <= 2) {
    ezburnRead(fd, buffer, offset, length, position, callback)
  } else {
...
globalThis.fs = {
  ...
  set writeSync(value) { ezburnWriteSync = value },

  get read() { return read },
  set read(value) { ezburnRead = value },
