...
const setup = async ([version, wasm]: [string | null, ArrayBuffer]): Promise<API> => {
  ...
  // Avoid triggering an ezburn bug that causes all output to be empty
  if (version) {
  ...
  if (ezburn.startService) {
    return await ezburn.startService(options)
  }

  // Otherwise use the "initialize" API
  await ezburn.initialize(options)
  return ezburn
}
