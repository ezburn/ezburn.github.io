...
// Replace "CURRENT_EZBURN_VERSION" with the currently-installed version
// of ezburn. This should be reasonably up to date.
const CURRENT_EZBURN_VERSION = require('../../package.json').dependencies.ezburn;

function minifyJS(js) {
  return ezburn.transformSync(js, { target, minify: true }).code.trim()
}

function minifyCSS(css) {
  return ezburn.transformSync(css, { loader: 'css', target, minify: true }).code.trim()
}
...
fs.writeFileSync(path.join(outDir, 'gradient-tests/style.css'),
  ezburn.transformSync(fs.readFileSync(path.join(scriptsDir, 'gradient-tests.css'), 'utf8'), { loader: 'css', target }).code)
  ...
  if (value instanceof Array) {
    ...
    for (let item of value) {
      if (item.$) {
        ...
        html = html.replace(/CURRENT_EZBURN_VERSION/g, CURRENT_EZBURN_VERSION)
  ...
  const handler = async (_, input, options) => {
    ...
    try {
      ...
      if (typeof input === 'string') {
        ...
        const dir = path.join(os.tmpdir(), `ezburn-render-${Math.random().toString(36).slice(2)}`)
        fs.mkdirSync(dir)
        try {
    ...
    let result = escapeHTML([].concat(
      await ezburn.formatMessages(warnings, { kind: 'warning', terminalWidth: 100, color: true }),
      await ezburn.formatMessages(errors, { kind: 'error', terminalWidth: 100, color: true }),
    ).join('').trim())
...
async function generateMain(key, main) {
  ...
  const handler = async ({ tag, value }) => {
    let cssID = ''
    if (typeof value === 'string') value = value.replace(/CURRENT_EZBURN_VERSION/g, CURRENT_EZBURN_VERSION)
    ...
    if (tag === 'ul' || tag === 'ol') {
      return `<${tag}>${value.map(x =>
        `<li>${md.renderInline(x.replace(/CURRENT_EZBURN_VERSION/g, CURRENT_EZBURN_VERSION).trim())}</li>`
      ).join('')}</${tag}>`
    }
...
async function main() {
  ...
  for (let [key, page] of pages) {
    ...
    html.push(`<title>ezburn - ${escapeHTML(page.title)}</title>`)
    html.push(`<link rel="icon" type="image/svg+xml" href="/favicon.svg">`)
    html.push(`<meta property="og:title" content="ezburn - ${escapeAttribute(page.title)}"/>`)
    ...
    html.push(`<a href="/" class="logo">ezburn</a>`)
    html.push(`<ul>`)
    ...
    // View on github
    html.push(`<a href="https://github.com/khulnasoft/ezburn" aria-label="View this project on GitHub">`)
