...
const version = JSON.parse(packageJSON).dependencies.ezburn
...
async function checkCli(text, value) {
  return await checkCommon('cli', text, value, async ({ testDir, writeFiles }) => {
    await execAsync(`npm i --prefer-offline ezburn@${version}`, { cwd: testDir, stdio: 'pipe' })
    await writeFiles()
...
async function checkGo(text, value) {
  return await checkCommon('go', text, value, async ({ testDir, writeFiles }) => {
    ...
    await fs.writeFile(path.join(testDir, 'go.mod'), `
      ...
      go 1.16
      require (
        github.com/khulnasoft/ezburn v${version}
      )
    `)
    await execAsync(`go mod download github.com/khulnasoft/ezburn@v${version}`, { cwd: testDir, stdio: 'pipe' })
